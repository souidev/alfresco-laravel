...
namespace Souidev\AlfrescoLaravel\Models;

use Illuminate\Http\UploadedFile;

use Souidev\AlfrescoLaravel\Models\Vendor\Zip\TbsZip;

use Souidev\AlfrescoLaravel\Models\Vendor\Cmis\CmisService;
use Souidev\AlfrescoLaravel\Models\Vendor\Cmis\Exceptions\CmisInvalidArgumentException;
use Souidev\AlfrescoLaravel\Models\Vendor\Cmis\Exceptions\CmisObjectNotFoundException;
use Souidev\AlfrescoLaravel\Models\Vendor\Cmis\Exceptions\CmisPermissionDeniedException;
use Souidev\AlfrescoLaravel\Models\Vendor\Cmis\Exceptions\CmisNotSupportedException;
use Souidev\AlfrescoLaravel\Models\Vendor\Cmis\Exceptions\CmisConstraintException;
use Souidev\AlfrescoLaravel\Models\Vendor\Cmis\Exceptions\CmisRuntimeException;
use Souidev\AlfrescoLaravel\Models\Vendor\Cmis\Exceptions\CmisNotImplementedException;

use Souidev\AlfrescoLaravel\Models\AlfrescoCmisObject;
use Souidev\AlfrescoLaravel\Models\AlfrescoDocument;
use Souidev\AlfrescoLaravel\Models\AlfrescoFolder;
use Souidev\AlfrescoLaravel\Exceptions\AlfrescoConnectionException;
use Souidev\AlfrescoLaravel\Exceptions\AlfrescoObjectNotFoundException;
use Souidev\AlfrescoLaravel\Exceptions\AlfrescoObjectAlreadyExistsException;
use Souidev\AlfrescoLaravel\Models\Helpers\AlfrescoHelper;
...
class AlfrescoCmisProvider
{
	...
	public function __construct($settings=false) {
		...
		if(!AlfrescoHelper::ends_with($this->rootpath,"/")) $this->rootpath.="/";
		...
		if(!AlfrescoHelper::ends_with($this->alfrescourl,"/")) $this->alfrescourl.="/";
	...
	private function checkInBaseFolder($object){
		...
		if(!AlfrescoHelper::starts_with($object->path, $this->getBasepath(true))){
			return true;
		}else {
...
/**
	...
	public function setBasepath($path){
		$this->basepath=$path;
		if(!AlfrescoHelper::ends_with($this->basepath,"/")) $this->basepath.="/";
	}
	...
	public function rename($objectId, $newName){// throws AlfrescoObjectNotFoundException, AlfrescoObjectAlreadyExistsException{
		...
		try{
			...
			if($obj->isDocument()){
				if(!AlfrescoHelper::ends_with($newName,".".$obj->extension)) $newName.=".".$obj->extension;
